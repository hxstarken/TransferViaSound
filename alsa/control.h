...
typedef enum _snd_ctl_event_type
{
	...
	SND_CTL_EVENT_ELEM = 0, SND_CTL_EVENT_LAST = SND_CTL_EVENT_ELEM
...
snd_ctl_elem_iface_t snd_ctl_event_elem_get_interface(
...
int snd_ctl_ascii_value_parse(snd_ctl_t *handle, snd_ctl_elem_value_t *dst,
		snd_ctl_elem_info_t *info, const char *value);
...
void snd_ctl_elem_list_get_id(const snd_ctl_elem_list_t *obj, unsigned int idx,
		snd_ctl_elem_id_t *ptr);
unsigned int snd_ctl_elem_list_get_numid(const snd_ctl_elem_list_t *obj,
		unsigned int idx);
snd_ctl_elem_iface_t snd_ctl_elem_list_get_interface(
		const snd_ctl_elem_list_t *obj, unsigned int idx);
...
snd_ctl_elem_iface_t snd_ctl_elem_info_get_interface(
...
int snd_ctl_add_integer_elem_set(snd_ctl_t *ctl, snd_ctl_elem_info_t *info,
		unsigned int element_count, unsigned int member_count, long min,
		long max, long step);
int snd_ctl_add_integer64_elem_set(snd_ctl_t *ctl, snd_ctl_elem_info_t *info,
		unsigned int element_count, unsigned int member_count, long long min,
...
int snd_ctl_add_enumerated_elem_set(snd_ctl_t *ctl, snd_ctl_elem_info_t *info,
		unsigned int element_count, unsigned int member_count,
		unsigned int items, const char * const labels[]);
...
snd_ctl_elem_iface_t snd_ctl_elem_value_get_interface(
...
int snd_ctl_get_dB_range(snd_ctl_t *ctl, const snd_ctl_elem_id_t *id, long *min,
		long *max);
 ...
 */
typedef int (*snd_hctl_callback_t)(snd_hctl_t *hctl, unsigned int mask,
...
int snd_hctl_poll_descriptors(snd_hctl_t *hctl, struct pollfd *pfds,
		unsigned int space);
